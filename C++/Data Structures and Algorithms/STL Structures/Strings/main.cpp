...
void print(std::string& name){}		        // std::string& is an lvalue-reference
...
int main(int argc, const char* argv[])
{
	...
	std::cout << name.substr(7, 12) << std::endl;					// substr(index, length) indexing starts at 0, 'Hagler'
	std::cout << name.replace(0, 6, "Joe") << std::endl;			// replace(index, length)

	std::string first = "Hello ";
	std::string second = "Joe";
	//print(first + second);										// compile error: cannot bind non-const lvalue reference to an rvalue (first + second)
