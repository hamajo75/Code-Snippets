...
$(function() {
    ...
    $('<div id="minitoc-home" class="dontprint"></div>').prependTo('body');
});

// generate contents of minitoc HOME
function generateMiniHomeToc(divId) {
    $('#minitoc-home').empty().append('<h2>HOME</h2>');

    $('#' + divId).find('h3').each(function(i) 
    {
        $("#minitoc-home").append("<a href='#" + $(this).attr("id") + "'>"
                             + $(this).text() + "</a>");
    });
    // Ensure that the target is expanded (hideShow)
    $('#minitoc-home a[href^="#"]').click(function() {
