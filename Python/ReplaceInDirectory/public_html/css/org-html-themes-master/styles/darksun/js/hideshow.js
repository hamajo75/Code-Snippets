...
function returnNextOutlineTextId(node)
{    
    if (node == null)
        return "";
    ...
    if (node.className.substring(0, 13) == "outline-text-")
        ...
        return returnNextOutlineTextId(node.parentNode);
}
// Expands an anchor, i.e. expand all parent headers
function hsExpandAnchor(id) {
    if (id.substring(0, 5) == "#fnr.")
        id = '#' + returnNextOutlineTextId(document.getElementById(id.substring(1, id.length)).parentNode);
