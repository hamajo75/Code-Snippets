...
function hsExpand(header) {
    hsExpand2(header, true, false);
}

// Expand a header
function hsExpand2(header, expandDoneHeader, clicked) {
    ...
    // update url (for copying)
    if (clicked && header.parent().hasClass(HS_CLASS))
        document.location.hash = header.parent().attr("id");
}
...
function hsCollapse(header){
    hsCollapse2(header, false);
}

// Collapse a header
function hsCollapse2(header, clicked) {
    // Ignore non collapsible entries
    if (!header.parent().hasClass(HS_CLASS)) return;
    ...
    // update url (for copying)
    if (clicked && header.parent().parent().hasClass(HS_CLASS))
        document.location.hash = header.parent().parent().attr("id");
}

// Toggle a header
function hsToggleCollapsing(header) {
    if (header.parent().hasClass('hsCollapsed'))
        hsExpand2(header, false, true);
    else if (header.parent().hasClass('hsExpanded'))
        hsCollapse2(header, true);
}

// Expand all headers
function hsExpandAll() {
    $('#content .hsCollapsed').each(function() {
        hsExpand2($(this).children(':header'), HS_SHOW_ALL_OPEN_DONE_TREES, false);
    });
}
...
// Add click events to H3/H4/H5/H6 headers which have contents.
function hsInit() {
    for (var i = 3; i <= 6; i++) {
...
function returnNextOutlineId(node)
{    
    if (node == null)
        return "";

    //if (node.className.substring(0, 13) == "outline-text-")
    if (node.className.substring(0, 8) == "outline-")
        return node.id;
    else
        return returnNextOutlineId(node.parentNode);
}
// Expands an anchor, i.e. expand all parent headers
// document.getElementById(id.substring(1,id.length)).className == "footref"
// className == "footnum"
function hsExpandAnchor(id) {    
    if (document.getElementById(id.substring(1,id.length)).className == "footref")
        id = '#' + returnNextOutlineId(document.getElementById(id.substring(1, id.length)).parentNode);
